  ...
  return (
    <div className="flex flex-col justify-evenly h-full gap-[1vh] min-h-0 py-[1vh] md:py-4">
      {steps.map((step, index) => (
        ...
        >
          <div
            className={`p-[1vh] rounded-xl bg-white/40 shadow-sm shrink-0 ${step.color || 'text-black dark:text-white'}`}
          >
            <step.icon className="w-[clamp(1.2rem,3vh,1.8rem)] h-[clamp(1.2rem,3vh,1.8rem)]" />
          </div>
          <div className="min-w-0 flex-1">
            <h3
              className={`font-black leading-none ${step.color || 'text-black dark:text-gray-100'} text-[clamp(1rem,2.5vh,1.5rem)]`}
            >
              {step.title}
            </h3>
